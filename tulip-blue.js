const canvas = document.getElementById("tulip-scene");
const ctx = canvas.getContext("2d");

const IMAGE_SOURCE = "image.png";
const DOT_COUNT = 42000;
...
function buildDots(image) {
  ...
  for (let i = 0; i < DOT_COUNT; i += 1) {
    ...
    const depthScatter = (Math.random() - 0.5) * (46 + bulge * 74);
    const openZ = pixel.mark * 34 + bulge * 52 - backLayer * (86 + bulge * 112) + depthScatter;
    ...
    dots.push({
      ...
      budX,
      budY,
      budZ: openZ * (0.28 + petalMotion * 0.2) + (1 - backLayer) * 24,
      petalMotion,
...
function getDotSprite(depthBucket) {
  ...
  gradient.addColorStop(0, "rgba(235, 249, 255, 1)");
  gradient.addColorStop(0.22, `rgba(${Math.round(55 + depth * 70)}, ${Math.round(135 + depth * 85)}, 255, 0.94)`);
  gradient.addColorStop(1, "rgba(0, 48, 190, 0)");
...
function draw(time = 0) {
  ...
  bg.addColorStop(0, "#09285e");
  bg.addColorStop(0.48, "#03132e");
  bg.addColorStop(1, "#00040d");
  ctx.fillStyle = bg;
  ...
  const autoTurn = Math.sin(seconds * 0.32) * (Math.PI / 6);
  const angleY = autoTurn + pointerX * 0.12 + breath * 0.7;
  ...
  for (const dot of projected) {
    const depth = Math.max(0, Math.min(1, (dot.z + 155) / 310));
